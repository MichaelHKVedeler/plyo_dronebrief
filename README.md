...
- PDF export wizard for local briefs, cloud projects, public viewers and portable snapshots, with English/Norwegian layouts, optional property information and instructions, map pages, point details, and reference images. Optional note edits use the active local/cloud save workflow; viewer notes remain PDF-only.
...
- Editor/viewer fit the browser window. The project title and Editor/Read-only badge sit in the top bar beside the logo and theme toggle, leaving more height for the map and Project/Contents panel. Long titles truncate without hiding header actions. The details panel has its own shadcn scrollbar; on narrow screens it sits below the map and the screen scrolls to keep controls accessible.
...
Choose **Export as PDF** in either the editor or viewer. The three-step wizard selects English or Norsk bokmål, suggests a nearby street address, reviews property information and instructions, and prepares the download. The cover shows **Street address: Area**, followed by project name and client. Google reverse geocoding uses the circle rig's center (or the first camera, then project coordinates if no rig exists). The suggested address is editable and applies only to this PDF; typing is never overwritten by a delayed lookup. If lookup is unavailable, enter a custom address or retry. Empty notes are optional. In the editor you can save revised notes to the brief when exporting; viewer notes apply only to the PDF. Choosing Cancel leaves the brief unchanged.
