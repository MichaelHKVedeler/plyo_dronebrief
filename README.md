...
Every time is a range with two handles. The track fills between them. Click either handle or its time label to choose the instant shown in ShadeMap. Left-drag a handle, or the active slider's track, to move the selected endpoint; endpoints cannot cross. Arrow keys move the focused endpoint by 15 minutes. Escape, pointer cancellation or losing window focus cancels an unfinished drag.

Up to three same-day ranges (00:00–23:59) can be stacked. Each range counts as one shoot for image totals. A saved time without an end is shown from its start through 17:00 when that is later, and editing saves that range. Editor changes save on release and survive reopening and share-key export. The viewer can preview ranges without saving. The selected handle and shadow preview are temporary view state. On narrow maps, the clock button opens a larger time editor with the same controls.
...
Open **Contents → Floor plan → Upload image** and choose a JPG or PNG (up to 30 MB and 100 million pixels, maximum ten images). The image starts at the current map center, sized to the viewport with its aspect ratio preserved. Grab an edge to move it. Right-click anywhere on the map to set the selected image’s anchor, or left-drag the anchor itself to move it. Dragging inside the image rotates and scales it together around that anchor. With no custom anchor, the image center is used. Select an image by clicking it or its name in Floor plan. Escape, pointer cancellation, or leaving the page cancels an unfinished drag. Middle-button panning and camera placement still work over images.
...
The cover wizard has independent **Include project name on cover** and **Include client name on cover** switches. Either or both can be omitted; the address stays visible and saved project details are unchanged. The Time Slot page and wizard review list the saved shoot times and ranges. They do not label sunrise, daytime, sunset or blue hour.

The landscape PDF follows this order: cover; shoot times; project size and capture requirements; compact points and coordinates; property information and instructions; brief maps with/without a floor plan; optional references; and the framed Points / Files page with RAW/JPG delivery guidance. Coordinate tables include the rig center, every rig arrow, and all camera points, with up to 40 rows per page. Long notes and point lists continue onto additional pages. Standard headings and instructions are translated; authored notes are preserved as entered.
...
Map export frames the entire scene, includes every geometry layer, and restores the previous view afterward. PDF map pages always use Google Maps, including while ShadeMap is open in the editor; the satellite setting and map dimming on that Google map are reflected in the capture. When a floor plan exists, the PDF contains maps with and without it; reconnect any missing local images first. Camera directions and 360 focus are shown on the map and in the coordinate tables. **Point diagram** is an explicit alternative without a basemap or floor plan, available even when map services cannot load. Up to four optional JPG/PNG reference images with captions are authored in Contents and included automatically.
