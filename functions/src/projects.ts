...
import { cloudId, summarySchema, type Actor, type CloudProject, type AssetManifest } from '../../src/features/cloud/model/cloud.js'
...
export async function createProject(user: Actor, input: unknown): Promise<CloudProject> {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const stamp = now()
    const summary = { id, orgId: data.orgId, name: data.brief.project.name, clientName: data.brief.project.clientName, createdBy: actorOnly(user), editedBy: actorOnly(user), createdAt: stamp, updatedAt: stamp, revision: 1, deletedAt: null, collectionId: null, collectionName: '' }
...
export async function saveProject(user: Actor, input: unknown) {
  ...
  return db.runTransaction(async (tx) => {
    ...
    const summary = { ...previous, name: data.brief.project.name, clientName: data.brief.project.clientName, editedBy: actorOnly(user), updatedAt: now(), revision: previous.revision + 1 }
