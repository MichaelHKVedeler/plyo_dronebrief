...
export async function pdfReference(file: File, caption: string): Promise<PdfReference> {
  ...
  try {
    const image = new Image(); image.src = local.url; await image.decode()
    const scale = Math.min(1, 2000 / Math.max(local.width, local.height))
    const canvas = document.createElement('canvas')
    canvas.width = Math.max(1, Math.round(local.width * scale)); canvas.height = Math.max(1, Math.round(local.height * scale))
