...
it('selects cameras and only commits their final dragged position', () => {
  ...
  const button = screen.getByRole('button', { name: 'Move Camera' })
...
it('supports keyboard aiming and leaves viewer arrows visible but disabled', () => {
  ...
  fireEvent.keyDown(screen.getByRole('button', { name: 'Aim Camera' }), { key: 'ArrowRight' })
  ...
  expect(screen.queryByRole('button', { name: 'Move Camera' })).toBeNull()
  const arrow = screen.getByRole('button', { name: 'Aim Camera' })
