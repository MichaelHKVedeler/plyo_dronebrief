  ...
  return <div className="grid gap-2">
    <div className="grid grid-cols-[minmax(0,1fr)_2rem] items-center gap-2">
      <div className="flex min-w-0 items-center gap-2">
        {date && <>
          <button type="button" className="h-8 shrink-0 text-left" aria-label="Shadow date text"
            onClick={() => dateInput.current?.showPicker?.()}>
            <time dateTime={date}>{formatSlotDate(date)}</time>
          </button>
          ...
          {slots.map((slot, index) => {
            ...
            return <li key={index}>
              <button type="button" className={index === activeIndex ? 'tabular-nums' : 'tabular-nums opacity-50'}
                ...
                <time dateTime={`${slot.date}T${slot.time}`}>{time.actualTime}</time>
              </button>
            </li>
          })}
        </ul>}
    ...
    <div id={panelId} hidden={!open} className="grid gap-2">
      {slots.map((slot, index) => {
        ...
        return <div key={index} className={index === activeIndex ? 'grid gap-1' : 'grid gap-1 opacity-50'}
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_2rem] items-center gap-2 text-sm">
            <strong className="tabular-nums">{time.actualTime}</strong>
