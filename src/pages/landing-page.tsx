...
export function LandingPage({ onCreate, onLoad, onResume, draft, error }: Props) {
  ...
  return <main className="mx-auto flex min-h-[calc(100svh-70px)] max-w-3xl flex-col justify-center gap-7 px-6 py-12">
    <div><h1 className="text-3xl font-semibold tracking-tight">Plan your next shoot.</h1></div>
    {error && <Alert variant="destructive"><AlertDescription>{error}</AlertDescription></Alert>}
    <div className="grid gap-4 sm:grid-cols-2">
      <Card><CardHeader><CardTitle>New brief</CardTitle></CardHeader><CardContent><Button onClick={onCreate}><Plus /> Create a brief</Button></CardContent></Card>
      <Card><CardHeader><CardTitle>Open a brief</CardTitle></CardHeader><CardContent><Button variant="outline" onClick={() => setLoadOpen(true)}><FolderOpen /> Load a brief</Button></CardContent></Card>
    </div>
    ...
    <Dialog open={loadOpen} onOpenChange={setLoadOpen}><DialogContent aria-describedby={undefined}>
      <DialogHeader><DialogTitle>Load a brief</DialogTitle></DialogHeader>
