...
it('adds a local image to a hidden layer, saves metadata, adjusts visibility, and recovers after reopening', async () => {
  ...
  await user.click(screen.getByRole('switch', { name: 'Image overlays' }))
  ...
  expect(screen.getByRole('switch', { name: 'Image overlays' })).toHaveAttribute('aria-checked', 'true')
...
it('reconnects local images in a viewer without saving or permitting edits', async () => {
  ...
  await user.click(within(screen.getByRole('tabpanel', { name: 'Layers' })).getByRole('switch', { name: 'Image overlays' }))
